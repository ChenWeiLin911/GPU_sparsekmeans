...
def main():
    ...
    file = open(result_dir / f"memoryusageBX_SPGEMM.txt", "w")
    ...
    before = cp.get_default_memory_pool().used_bytes()
    gpu_B = csp.csr_matrix(B)
    after = cp.get_default_memory_pool().used_bytes()
    file.write(f"allocate B: {(after - before)/ (1024**2)} MB\n")
    before = cp.get_default_memory_pool().total_bytes()
    D = gpu_B @ gpu_X
    after = cp.get_default_memory_pool().total_bytes()
    file.write(f"SpGEMM cost: {(after - before) / (1024 ** 2)} MB\n")
    file.write(f"D cost: {csr_matrix_size(D) / (1024 ** 2)} MB\n")
    file.write(f"Buffer cost:{(after - before - csr_matrix_size(D)) / (1024 ** 2)} MB")
    file.close()
